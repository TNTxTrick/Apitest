...
exports.index = async (req, res, next) => {
    ...
    try {
        ...
        $('div.w-full.relative').each((index, element) => {
            const titleArray = $(element).find('div.latest-chapter.truncate a').text().trim().split('\n').map(t => t.trim()).filter(Boolean);
            const chapterArray = $(element).find('div.p-2 a').map((i, el) => $(el).text().trim()).get();
            const updateArray = $(element).find('div.p-2 span').map((i, el) => $(el).text().trim()).get();
            const linkArray = $(element).find('div.relative a').map((i, el) => $(el).attr('href').trim()).get();
